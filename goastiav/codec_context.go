...
static inline uint64_t get_channel_layout_mask(const AVChannelLayout *layout) {
	//  AV_CHANNEL_ORDER_NATIVE 에만 사용해야 함
...
func (cc *CodecContext) SetChLayoutDefault(nbChannels int) {
	C.av_channel_layout_default(&cc.c.ch_layout, C.int(nbChannels))
}
...
func (cc *CodecContext) ChannelLayoutMask() uint64 {
	return uint64(C.get_channel_layout_mask(&cc.c.ch_layout))
}
...
func (cc *CodecContext) SendPacket(p *Packet) error {
	...
	ret := C.avcodec_send_packet(cc.c, pc)
	return newError(ret)
}
...
func (cc *CodecContext) ReceiveFrame(f *Frame) error {
	...
	ret := C.int(C.avcodec_receive_frame(cc.c, fc))
	return newError(ret)
	//return C.avcodec_receive_frame(cc.c, fc)
}
