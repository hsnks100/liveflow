...
func (f *Frame) AllocBuffer(align int) error {
	if f.c == nil {
		return errors.New("Frame struct is not initialized")
	}
	ret := C.av_frame_get_buffer(f.c, C.int(align))
	if ret < 0 {
		return newError(ret)
	}
	return nil
...
func (f *Frame) SetChLayoutMask(mask uint64) error {
	res := C.av_channel_layout_from_mask(&f.c.ch_layout, C.uint64_t(mask))
	if res < 0 {
		return fmt.Errorf("failed to set channel layout mask: %d", int(res))
	}
	return nil
}
